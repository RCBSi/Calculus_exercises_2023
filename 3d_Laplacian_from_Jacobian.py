import pandas, numpy
Jacobian_string_1 = """\cos(\phi_1) & \sin(\phi_1) \cos(\phi_2) & \sin(\phi_1) \sin(\phi_2)
-\sin(\phi_1) \dfrac{1}{r} & \cos(\phi_1) \cos(\phi_2) \dfrac{1}{r} & \cos(\phi_1) \sin(\phi_2) \dfrac{1}{r}
0& -\sin(\phi_2) \dfrac{1}{r} \dfrac{1}{\sin(\phi_1)} & \cos(\phi_2) \dfrac{1}{r} \dfrac{1}{\sin(\phi_1)}"""
repdi = {'-':'-1 ', '\cos(\phi_1)':'c_1', '\sin(\phi_1)':'s_1', 'r ': 'r_0 ', 
    '\cos(\phi_2)':'c_2','\sin(\phi_2)':'s_2', '\cdot':'',
    '\dfrac{1}{r}':'r^{-1}_0', '\dfrac{1}{s_1}': 's^{-1}_1' }
...
Jac_part = [[JacobT[i2][i3]+' z_'+str(i3) for i3 in range(3)] for i2 in range(3)]
summands = []
for row in Jac_part:
    for x in row:
        for y in row:
...
for x in gather:
    print('$$('+gather[x]+')'+' '.join(
        ['\dfrac{\partial}{\partial '+{'0':'r', '1':'\phi_1', '2':'\phi_2'}[x4]+'}' for x4 in [x3[-1] for x3 in x.split(' ')]]
    )+'$$' 
    )
